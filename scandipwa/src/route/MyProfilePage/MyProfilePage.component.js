...
export class MyProfilePageComponent extends PureComponent {
    ...
    renderAccountManagerInfoBlock() {
        const { accountmanageremail } = this.props;

        return (
            <div>
                <h2>{ __('Account manager') }</h2>
                <div>
                    <p>{ __('Email:') }</p>
                    <p>{ accountmanageremail }</p>
                </div>
            </div>
        );
    }

    renderAreaManagerInfoBlock() {
        const { accountmanageremail } = this.props;

        return (
            <div>
                <h2>{ __('Area manager') }</h2>
                <div>
                    <p>{ __('Email:') }</p>
                    <p>{ accountmanageremail }</p>
                </div>
    ...
    renderSecondColumn() {
        return (
            <div>
                ...
                { this.renderAccountManagerInfoBlock() }
                { this.renderAreaManagerInfoBlock() }
            </div>
        );
    }
